-- Quick fix for missing problem_solving_score column and interview_results table
-- Run this in Supabase SQL Editor

-- Add missing problem_solving_score column to interview_sessions table
ALTER TABLE public.interview_sessions 
ADD COLUMN
IF NOT EXISTS problem_solving_score DECIMAL
(3,2) 
CHECK
(problem_solving_score >= 0 AND problem_solving_score <= 10);

-- Create interview_results table if it doesn't exist
CREATE TABLE
IF NOT EXISTS public.interview_results
(
  id UUID DEFAULT gen_random_uuid
() PRIMARY KEY,
  interview_session_id UUID REFERENCES public.interview_sessions
(id) ON
DELETE CASCADE,
  user_id UUID
...
DELETE CASCADE,
  job_role_id UUID
REFERENCES public.job_roles
(id) ON
DELETE
SET NULL
,
  job_role_title VARCHAR
(100) NOT NULL,
